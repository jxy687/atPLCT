## 文档
### 使用 RuyiSDK 构建项目
- **PVZ**：[构建文档](https://github.com/jxy687/Docs/blob/main/PVZ/main.md)
    - 使用 RuyiSDK 创建虚拟环境并构建项目，成功运行
    - 项目源代码 [PvZ-Portable](https://github.com/wszqkzqk/PvZ-Portable.git)

## 博客
### 在 ruyisdk.cn 上
- **xscc**：[博客](https://ruyisdk.cn/t/topic/2562)
    - XSCC 快速上手：基于 RuyiSDK 的游戏编译
- **太空射击**：[博客](https://ruyisdk.cn/t/topic/2565)
    - 基于 RuyiSDK 搭建虚拟环境，通过配置 sysroot、安装并编译 SDL 系列等依赖，完成太空射击游戏 SDLShooter 的交叉编译与模拟运行
